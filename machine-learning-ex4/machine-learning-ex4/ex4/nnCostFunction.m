...
reg_term = (lambda/(2*m))* (sum(sumsq(Theta1(:, 2:end))) + sum(sumsq(Theta2(:, 2:end)))) ;
J += reg_term;
...
d3 = a3 -Y;
d2 = (d3*Theta2).*[ones(size(z2), 1) sigmoidGradient(z2)];
...
% Add regularization of the Gradient
